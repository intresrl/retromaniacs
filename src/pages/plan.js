    ...
    return <div>
      ...
      <div style={{display: 'flex', padding: "5px", textTransform: "none", overflow: "auto"}}>
        {card1 ? <><Card card={card1}/><Icon onClick={() => setCard1(null)}>close</Icon></> :
          data.allCardsYaml.nodes.map(card => <Card card={card} onClick={() => setCard1(card)} stage="set-the-stage"/>)
        }
      </div>
      <h2>Gather data</h2>
      <div style={{display: 'flex', padding: "5px", textTransform: "none", overflow: "auto"}}>
      ...
      <div style={{display: 'flex', padding: "5px", textTransform: "none", overflow: "auto"}}>
        {card3 ? <><Card card={card3}/><Icon onClick={() => setCard3(null)}>close</Icon></> :
          data.allCardsYaml.nodes.map(card => <Card card={card} onClick={() => setCard3(card)} stage="generate-insight"/>)
        }
      </div>
      <h2>Decide what to do</h2>
      <div style={{display: 'flex', padding: "5px", textTransform: "none", overflow: "auto"}}>
        {card4 ? <><Card card={card4}/><Icon onClick={() => setCard4(null)}>close</Icon></> :
          data.allCardsYaml.nodes.map(card => <Card card={card} onClick={() => setCard4(card)} stage="decide-what-to-do"/>)
        }
      </div>
      <h2>Close the retrospective</h2>
      <div style={{display: 'flex', padding: "5px", textTransform: "none", overflow: "auto"}}>
        {card5 ? <><Card card={card5}/><Icon onClick={() => setCard5(null)}>close</Icon></> :
          data.allCardsYaml.nodes.map(card => <Card card={card} onClick={() => setCard5(card)} stage="close-the-retrospective"/>)
        }
      </div>
    </div>
}
